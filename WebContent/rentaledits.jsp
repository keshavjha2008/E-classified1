...
function emp()
{
	...
	if(jobcodeedit.t7.value != ""){	  
		  if (isNaN(jobcodeedit.t7.value)) { 
			    alert("Please enter valid number");
			    jobcodeedit.t7.focus() ;
			    return false;
			} 	
	  }
...
<%
	try {
	   ResultSet rs=st.executeQuery("select * from rentals where houseno="+jcodeedit);
	    if(rs.next())
	     {
...
<form name="jobcodeedit" action="rentaledit.jsp" method="post" onsubmit="return emp();">
...
<TD>Address</TD>
 ...
 <td>
    <input type="text"   name="t9" size="20" value="<%=rs.getString(9)%>" tabindex="8" >
          </TD>
